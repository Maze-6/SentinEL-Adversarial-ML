...
class PhishModel:
    ...
    def train_model(self, csv_path: str):
        try:
            ...
            # Using Pipeline to scale data before passing to RF
            pipeline = Pipeline([
    ...
    def predict_vanguard(self, url, threshold):
        ...
        if clean_url in config.TRUSTED_DOMAINS:
             return "LEGITIMATE", 0.01, ["Verified Global Authority (Allowlist)"]
        
        if any(clean_url.endswith("." + trusted) for trusted in config.TRUSTED_DOMAINS):
            return "LEGITIMATE", 0.01, ["Verified Subdomain of Trusted Authority"]

        # 2. Extract Features
        feats = FeatureExtractor.extract_features(url)
        
        # 3. DNS Verification
        if feats[8] == 0: return "PHISHING", 1.0, ["CRITICAL: Domain Resolution Failed"]

        # 4. Reputation Override (Age > 1 Year)
        if feats[9] > 365:
        ...
        if feats[9] == -1 and v != "LEGITIMATE": reasons.append("Forensic: WHOIS Privacy/Block Detected")
